...
class Engine(object):
    ...
    def get_free_adj(self, pos):
        new_pos = None
        mo = [(-1, -1), (-1, 0), (-1, 1), (1, -1), (1, 0), (1, 1), (0, -1), (0, 1)]

        random.shuffle(mo)
        while new_pos is None and len(mo) > 0:
            t = mo.pop()
    ...
    def create_character(self):
        ...
        while not OK:
            ...
            self.__render_text(
                self.screen, g[gender] + " " + races[race][0] + " " + classkits[classkit][0] + ".", GREEN, ((167, y))
            )
            y += 20
            ...
            self.__render_text(
                self.screen,
                classkits[classkit][1].desc.replace("$$$", title.lower()).replace("%%%", title2.lower()),
                WHITE,
                ((145, y)),
            )
    ...
    def _world_move(self):
        if not self.__quit_loop:
            if self.state == S_RUN:
                ...
                if self.__gfx is None:
                    self.state = S_RUN

    def _world_draw(self):

        if self.__gfx is None or self.__gfx.redraw:
                ...
                if act == self.player or self.player.sc.lit(act.x, act.y) and not self.player.dazzled:
                    try:
                        self.screen.blit(
                            ...
                            (act.x * TILESIZE - self.camera.x * TILESIZE, act.y * TILESIZE - self.camera.y * TILESIZE),
                        )
                        if act not in self.__actors_on_screen:
    ...
    def __get_map_surface(self):
        if self.map.cur_surf is None:
            ...
            for x in range(max(cx, 0), min(self.map.width, cw + 1)):

                for y in range(max(cy, 0), min(self.map.height, ch + 1)):
    ...
    def __get_statblock_surface(self):
        if self.__cur_stat_surf is None:
    ...
    def __draw_stat_block(self, surf):
        ...
        y = 55
        color = WHITE
        if self.player.HP.Head[0] == self.player.HP.Head[1]:
            color = GREEN
        if self.player.HP.Head[0] == 0:
            color = YELLOW
        if self.player.HP.Head[0] < 0:
            color = RED
        ...
        y = 75
        color = WHITE
        if self.player.HP.Chest[0] == self.player.HP.Chest[1]:
            color = GREEN
        if self.player.HP.Chest[0] == 0:
            color = YELLOW
        if self.player.HP.Chest[0] < 0:
            color = RED
        ...
        y = 85
        color = WHITE
        if self.player.HP.L_Arm[0] == self.player.HP.L_Arm[1]:
            color = GREEN
        if self.player.HP.L_Arm[0] == 0:
            color = YELLOW
        if self.player.HP.L_Arm[0] < 0:
            color = RED
        ...
        x = 110
        color = WHITE
        if self.player.HP.R_Arm[0] == self.player.HP.R_Arm[1]:
            color = GREEN
        if self.player.HP.R_Arm[0] == 0:
            color = YELLOW
        if self.player.HP.R_Arm[0] < 0:
            color = RED
        ...
        y = 130
        color = WHITE
        if self.player.HP.L_Leg[0] == self.player.HP.L_Leg[1]:
            color = GREEN
        if self.player.HP.L_Leg[0] == 0:
            color = YELLOW
        if self.player.HP.L_Leg[0] < 0:
            color = RED
        ...
        x = 95
        color = WHITE
        if self.player.HP.R_Leg[0] == self.player.HP.R_Leg[1]:
            color = GREEN
        if self.player.HP.R_Leg[0] == 0:
            color = YELLOW
        if self.player.HP.R_Leg[0] < 0:
            color = RED
        ...
        stats = [
            ("STR ", self.player.STR, self.player.get_STR()),
    ...
    def __gen_id(self):
        for x in range(self.__last_id, 9999999):
            yield x
